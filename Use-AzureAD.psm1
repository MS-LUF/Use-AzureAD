...
Function Get-AzureADAccessToken {
    ...
    <#
        ...
        -AdUserFilter string
        Ad filter attribute for limiting users found during search.
        Defaults to * if no filter is set.
        Must be valid adfilter. For Instance "userprincipalname -like '*@Domain.Com'"
        ...
        .Example
        Add azure ad users to Administrative Unit based on their source OU. Include only users that have a domain UPN of Domain.com and are contained in the myusers ou.
        The verbose option can be used to write basic message on console (for instance when a user is already member of an admin unit)
        C:\PS> Sync-ADUsertoAzureADAdministrativeUnitMember -CloudUPNAttribute UserPrincipalName -OUsDn 'OU=myusers,dc=domain,dc=com' -AduserFilter "userprincipalname -like '*@Domain.com'" -Verbose
    #>
        [cmdletbinding()]
        Param (
            [parameter(Mandatory=$true)]
